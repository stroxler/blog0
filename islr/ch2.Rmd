---
title: "Introduction to Statistical Learning 2: R programming and plotting functions"
...
This post, tagging along with Chapter 2, we will go through how to write functions in R and how to make plots of functions (as opposed to plots of data, which we covered last week).

Once we've gone through these basics, we'll walk through an example of an R program looking at the bias-variance trade-off in linear models via simulation.
...
If you want to plot a 1d function, you can use the simple `plot` function. By default, `plot` makes a scatterplot, but we can draw a curve by using `type = 'l'`:
```{r}
x = 1:100 / 100
plot(x, sqrt(x), type='l')
```

If we want to plot multiple functions side by side, we need to know two more plotting tools. The first is
that we can add color to a plot using the `col` argument:
```{r}
plot(x, x + 5, type='l', col='blue')
```

The second is that each call to `plot` redraws the window. If we want to add points or curves to a plot window we already made, we should use the `lines` or `points` functions:
```{r}
