...
Welcome to the sixth post in my series on [An Introduction to Statistical Learning with Applications in R](http://www-bcf.usc.edu/~gareth/ISL/). In the last few posts, we looked at regression and classification using several different models.

This post, we'll return to the horsepower vs miles per gallon estimation on the `Auto` dataset, and look at how to work with cross-validation in R.

## The Auto dataset

Let's start out by taking a look at the auto data:
```{r, message=FALSE}
library(ggplot2)
library(dplyr)
...
We see a strong relationship between horsepower and miles per gallon, but it isn't quite linear. We are going to use cross-validation to help us fit a polynomial to this data using linear regression. To do this, we'll use the `cv.glm` function from the `boot` package.
...
We can't easily evaluate a hyperparameter like the polynomial degree the training data, because the fit on the training data always gets better when we make the model more complex. For linear regression, we can try to use hypothesis tests or AIC to control for this, but in more complicated models we might not have even that.

But if we have a test dataset to measure fit on, this problem isn't so serious: by making an overly complex model and overfitting the training data, we'll see that our test data gets worse.

### Cross-validation folds

When a model is expensive to fit and there is a lot of data, often we might just split the data into a training and test set as described above. But when we do that, we lose some information because the split is arbitrary. In K-fold cross-validation, we partition the data into $K$ chunks instead, and we use each chunk as a training set once, while using the $K-1$ other chunks as the training data.

For example, in three-fold cross validation we would split our dataset into three equal chunks. We would take the first two chunks to train a model, then measure the error on the third chunk. Then we'd train another model on the first and last chunk, fitting it on the middle. Finally, we'd train on the last two chunks and fit on the first, and then add up all the errors.

One common type of cross-validation is leave-one-out cross validation, where $K = M$, the number of data points. For a complex model, this type of cross validation is very expensive since we have to fit the model many, many times. It also tends to be a slightly poorer estimate of fit, for reasons that have to do with the variance of the cross-validation error estimate. But as we'll see, for linear regression we can compute the leave-one-out cross validation error almost for free, which makes it appealing.
